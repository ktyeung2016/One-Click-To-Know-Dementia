...
static const NSString * LockViewKey             = @"LockViewKey";
...
- (void)makeToastActivity {
    [self makeToastActivity:CSToastActivityDefaultPosition withMessage:nil];
}

- (void)makeToastActivityMessage:(NSString *)message {
    [self makeToastActivity:CSToastActivityDefaultPosition withMessage:message];
}

- (void)makeToastActivity:(id)position withMessage:(NSString *)message {
    UIView *existingLockView = (UIView *)objc_getAssociatedObject(self, &LockViewKey);
    if (existingLockView != nil) return;
    
    UIView *lockView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height)];
    ...
    UIView *activityView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, (message == nil) ? CSToastActivityWidth : CSToastActivityWidth + 20, (message == nil) ? CSToastActivityHeight : CSToastActivityHeight + 20)];
    ...
    activityIndicatorView.center = CGPointMake(activityView.bounds.size.width / 2, (message == nil) ? (activityView.bounds.size.height / 2) : (activityView.bounds.size.height / 2 - 10));
    ...
    if (message != nil) {
        ...
        [label sizeToFit];
        [label setCenter:CGPointMake(activityIndicatorView.center.x, activityIndicatorView.center.y + 40)];
        [activityView addSubview:label];
    }
    
    // associate the activity view with self
    objc_setAssociatedObject (self, &LockViewKey, lockView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)hideToastActivity {
    UIView *existingLockView = (UIView *)objc_getAssociatedObject(self, &LockViewKey);
    if (existingLockView != nil) {
        [UIView animateWithDuration:CSToastFadeDuration
                              delay:0.0
                            options:(UIViewAnimationOptionCurveEaseIn | UIViewAnimationOptionBeginFromCurrentState)
                         animations:^{
                             existingLockView.alpha = 0.0;
                         } completion:^(BOOL finished) {
                             [existingLockView removeFromSuperview];
                             objc_setAssociatedObject (self, &LockViewKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                         }];
    }
